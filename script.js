document.addEventListener('DOMContentLoaded', () => {
    ...
    function downloadCanvas(format) {
        let canvas = codeOutput.querySelector('canvas');
        if (!canvas && activeTab === 'barcode') {
            const svg = codeOutput.querySelector('svg');
            if(svg) canvas = convertSvgToCanvasSync(svg);
        }
        if (!canvas) return;
    ...
    if (contactForm) contactForm.addEventListener('submit', (e) => {
        e.preventDefault(); const currentLang = document.documentElement.lang;
        if (!contactForm.checkValidity()) { alert(translations[currentLang].fillAllFields); return; }
